...
table <- df_nomtable %>%
    filter(p_value < setp_value) %>%
    mutate(p_value = scientific_format()(p_value)) %>%
    DT::datatable(
      caption = htmltools::tags$caption(
        style = "caption-side: top; text-align: left;",
        paste0("Significant (p < ", setp_value, ") ", type, " terms found in ", category, " differentiated genes")
      ),
      options = list(
        scrollX = TRUE,
        scrollY = "400px",
...
```{r show_GO_results, echo=FALSE, message=FALSE, warning=FALSE, results='asis', fig.height=5}
...
for (i in sig_contrast_list){
  ...
  cat("#####", i, "\n\n")  # adds a section heading in RMarkdown 
  print(res$table)
  print(res$plot)
